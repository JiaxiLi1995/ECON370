...
subtitle: "Lecture 7: Data cleaning & wrangling: Tidyverse"
author: "Jiaxi Li"
...
- There are some things that you'll have to venture outside of the tidyverse for (**loops, functions, etc.**).
...
We see that we have actually loaded a number of packages (these are **core** packages, which could also be loaded individually): **ggplot2**, **tibble**, **dplyr**, etc.
...
- However, bear in mind that these **optional** packages will have to be loaded separately.
...
There are more functions can be used with `select` such as `start_with()`, `end_with()`, `matches()` and `num_range()`.
...
What would happen if we remove "na.rm = TRUE"? What does ".groups = "drop" do?
...
`count`, `n_distinct` and `distinct`: Number and isolate unique observations.
- E.g. `starwars |> count(species)`, or `starwars |> n_distinct(species)`, or `starwars |> distinct(species)`
...
left_join(
  ...
  select(flight_year, month, day, dep_time, arr_time, carrier, flight, tailnum, year_built, type, model) |>
...
Now, please take one minute to work on the following question.
...
We can also pivot_longer and separate multiple variables. More info can be found  [here](https://stackoverflow.com/questions/69798752/pivot-longer-for-multiple-sets-having-the-same-names-to).

```{r stocks2, echo=FALSE}
