---
title: "Lecture 10 Script Template"
author: "Jiaxi Li"
date: "2025-11-11"
output: pdf_document
---
...
samp_means = cumsum(?(Nsim))/(1:Nsim)  # Compute cumulative average of uniform(0,1) draws
...
Nsamp        = ?                   # Sample size for each simulation
sample_means = rep(0, ?)          # Preallocate vector to store sample means
...
Change the Nsamp to see its effect to CLT. Change the Nsim to see almost no effect.
...
for(sim in 1:Nsim){
  e                 = ?(Nsamp)                # draw e values from N(0,1)
  x                 = ?(Nsamp)                # draw x values from N(0,1)
  y                 = ?                           # create y based on x, e, and the model
  sample_OLS[sim]   = coef(lm(y ~ x - 1))         # Estimate beta use ols without constant
  sample_lazy[sim]  = coef(lm(y? ~ x? - 1)) # Estimate beta use lazy
}
...
# Ridge regression example with lambda = 5 (alpha=?)
ridge_model = ?(x, y, alpha = ?, lambda = 5)
...
```{r}
?(lm(hp ~ mpg + wt + drat + qsec, data = mtcars))
```
...
for(k in 1:K){
  ...
  for(i in seq_along(lambda_grid)){
    ...
    # Mean squared error
    cv_errors[i, k] = mean((y_val - y_pred)^2)
  }
}
...
After finding the best $\lambda$ from one of the procedure above, you need to refit the model with $\lambda_{best}$ and all training + validation set.
...
If you used caret package, you can directly access the final model as below:
...
Let's use the code from econ370_final_project.Rmd to extract return data. Use Ridge regression for current return and 1-lag return as X's and future return as Y.
...
# Start from the end year 2009 to 2024 which are testing periods
