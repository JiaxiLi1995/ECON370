...
- Already installed: **dplyr**, **ggplot2**, **nycflights13**, **microbenchmark**
- New: **data.table**, **tidyfast**, **dtplyr**
...
The **tidyverse** is great. It can perform all kinds of data wrangling and **pipeline** makes the workflow very intuitive.
...
I'll lump these together, since they really have to do with the stability of your code over time. Just to emphasis the point about [dependencies](http://www.tinyverse.org/), though:
...
- `tidyverse`: uses verb functions, which makes **very clear workflows**.
- `data.table`: is **faster and more memory efficient**.

--

We'll get back to the point about complementary at the end of the lecture. 
...
We’ve already seen that the `tidyverse` offers an enhanced version of a data.frame called a **tibble**. Additionally, we can use `read_csv()` and `write_csv()` to efficiently read data into a tibble or write a tibble to a CSV file.
...
- Beyond simple visual enhancements (similar to tibbles), the specialized internal structure of data.table objects is a key reason why the package is so fast. (More [here](https://rdatatable.gitlab.io/data.table/articles/datatable-intro.html#what-is-datatable-1a) and [here](https://twitter.com/matloff/status/1131372631372918784).)

--

To create a data.table, we have a couple of options:
- `fread('mydata.csv')` efficiently reads a CSV into R as a data.table (`fwrite()` writes a CSV).
...
That last bullet leads us to an important concept that underlies much of data.table's awesomeness: It tries, as much as possible, to *modify by reference*. What does this mean?

--

A very (very) short answer is that there are basically two ways of changing or assigning objects in R:
1. **Copy-on-modify:** Generates a separate (deep) copy of your data before making changes, which can add computational overhead. (`dplyr` does this!)
2. **Modify-in-place:** Updates the data directly in memory without creating any copy.

I will make a simple demonstration on the blackboard.
...
Uh-oh! It too has been removed... exactly as modifying by reference demands. To avoid this behavior, use the explicit [`data.table::copy()`](https://rdatatable.gitlab.io/data.table/reference/copy.html) function. Run this next chunk yourself:
...
But if you prefer using pipe, that's also possible. Just prefix each step with `_`:
...
One subtle point is that using `merge()` to join two datasets will sort the result by the key by default. To preserve the original order of the first dataset-similar to `left_join()`-set sort = FALSE:
...
Now, please take one minute to work on the following question (same as the questions in previous lecture using `dplyr`).
...
Note that the created variable "stock" is a factor.

--

If you do not want a column of factors, you can set variable.factor = F,
...
- All of these ideas support data.table's core goals: Maximize performance and flexibility, whilst maintaining a concise and consistent syntax. They are worth learning.

Pro tip: Use keys to order your data and yield dramatic speed-ups.

The tidyverse and data.table are often viewed as substitutes, but you can profitably combine both into your workflow... even if you favor one for (most of) your data wrangling needs.

---

# Further resources

As hard as it may be to believe, there's a ton of data.table features that we didn't cover today. Hopefully, I've given you enough of a grounding to continue exploring on your own.
...
# Next lecture: Introduction to Data Science and Linear Model
<html><div style='float:left'></div><hr color='#EB811B' size=1px width=796px></html>
