---
title: "Lecture 4 Script Template"
author: "Jiaxi Li"
date: "2025-09-16"
output: pdf_document
---
...
Create a function for Pythagorean Theorem hypotenuse calculation. The inputs are base and height.
...
How to use the weighted mean function for simple mean?
...
#### Add Default in wt_mean and more
...
Exercise: print out "the remainder is r" for the integers 1 to 10 where r is the remainder when dividing by 3. For remainder 0, print "divisible".
...
```{r}
# Here, stop will print an error message
x = 0
if(x>0){
  print(1/x)
}else if(x<0){
...
Add same length requirement in wt_mean using `if` and `stop`
...
## III. Advanced Function Features
...
```{r}
# function calculates tax based on income
...
```{r}
# function calculates tax based on income, also effective tax rate
tax_bracket <- function(income) {
  if (income <= 10000) {
...
## IV. Functions which Generate Randomness
